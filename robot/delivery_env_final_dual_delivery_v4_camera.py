...
class DeliveryRobotEnv(gym.Env):
    ...
    # ============================================================
    #
    # Off by default — every existing curriculum stage and
    # checkpoint keeps working exactly as before. Call
    # env.set_dynamic_obstacles(True) to switch on bounded
    # back-and-forth patrol motion (pedestrian/vehicle-style)
    # for whatever obstacles the current task_mode places.
    #
    # Each obstacle patrols along a random direction, oscillating
    # smoothly around its randomized spawn point (its "anchor")
    # within a small radius — never drifting away, never needing
    # wall-bounce logic. This keeps the A*-validated spawn layout
    # meaningful while still requiring reactive avoidance, since
    # a patrolling obstacle can temporarily swing into what was,
    # at spawn time, a clear path.
    # ============================================================

    def set_dynamic_obstacles(self, enabled):

        self.dynamic_obstacles_enabled = bool(enabled)

        print()
        print("=" * 70)
        print(
            "DYNAMIC OBSTACLES:",
            "ON" if self.dynamic_obstacles_enabled else "OFF"
        )
        print("=" * 70)

    def _init_dynamic_obstacles(self):

        self.obstacle_anchor = []
        self.obstacle_direction = []
        self.obstacle_radius = []
        self.obstacle_period = []
        self.obstacle_phase = []

        # Only pedestrians and delivery vehicles patrol — a road
        # barrier or a stack of parcels does not move on its own
        # in real life. obstacle_asset_files[i] tells us which
        # physical type lives in slot i (this mapping is fixed
        # for the scene's lifetime; only x/y positions shuffle).
        MOVABLE_ASSETS = (
            "pedestrian_obstacle.urdf",
            "delivery_vehicle_obstacle.urdf",
        )

        for i, obstacle_data in enumerate(self.obstacle_data):

            x, y = obstacle_data[0], obstacle_data[1]

            self.obstacle_anchor.append((x, y))

            asset_file = (
                self.obstacle_asset_files[i]
                if i < len(self.obstacle_asset_files)
                else None
            )

            is_movable = asset_file in MOVABLE_ASSETS
            ...
            self.obstacle_direction.append(
                (
                    math.cos(angle),
                    math.sin(angle)
                )
            ...
            if is_movable:
                ...
                # How far (metres) each obstacle patrols from its
                # spawn point. Kept modest relative to the ~1.5-
                # 2.0m minimum obstacle separation used at spawn
                # time, so two patrolling obstacles very rarely
                # overlap.
                self.obstacle_radius.append(
                    np.random.uniform(0.35, 0.60)
                )

            else:

                # Road barrier / parcel stack: radius 0 means the
                # oscillation formula still runs every step but
                # always evaluates to "stay at the anchor" — no
                # special-casing needed elsewhere.
                self.obstacle_radius.append(0.0)

            # Full back-and-forth cycle takes 4-8 seconds —
            # slow, pedestrian/vehicle-like motion, not erratic.
            self.obstacle_period.append(
                np.random.uniform(4.0, 8.0)
            )

            self.obstacle_phase.append(
                np.random.uniform(0.0, 2.0 * math.pi)
            )

    def _update_dynamic_obstacles(self):

        # One env.step() = ACTION_REPEAT physics substeps at
        # dt=0.01s each.
        dt = self.ACTION_REPEAT * 0.01
        ...
        for i, obstacle_data in enumerate(
            ...
            if i >= len(self.obstacle_anchor):

                # Safety net: if obstacle count ever exceeds what
                # was initialized (shouldn't happen), leave any
                # extras static rather than crashing.
                new_layout.append(obstacle_data)
                continue

            _, _, sx, sy, sz = obstacle_data

            self.obstacle_phase[i] += (
                2.0
                *
                math.pi
                *
                dt
                /
                self.obstacle_period[i]
            )

            offset = (
                self.obstacle_radius[i]
                *
                math.sin(self.obstacle_phase[i])
            )

            ax, ay = self.obstacle_anchor[i]
            ux, uy = self.obstacle_direction[i]
            ...
            new_layout.append(
                (new_x, new_y, sx, sy, sz)
            )

        self.obstacle_data = new_layout

...
# ------------------------------------------------------------
# CROSS-PLATFORM "press N to skip episode" support
# ------------------------------------------------------------
# The original script used msvcrt, which only exists on Windows
# and crashes immediately on import anywhere else (Linux/Mac/
# Colab). This gives the same _kbhit()/_getch() interface on
# any OS. On Windows it's identical to before. On Linux/Mac it
# uses select() on stdin — this only detects a key AFTER Enter
# is pressed (no true raw single-keypress detection without
# extra setup), which is a minor UX downgrade but never crashes.
# If anything about this environment is unusual, it just quietly
# disables the skip feature instead of raising an error.
# ------------------------------------------------------------
...
# Set True to make obstacles patrol back-and-forth (pedestrian/
# vehicle-style) instead of staying fixed for the whole episode.
# The A* replanning + deadlock recovery below already re-checks
# the path periodically, so this is a natural stress test of
# that machinery — no other changes needed.
...
env.set_dynamic_obstacles(DYNAMIC_OBSTACLES)
